...
<h2> <?php echo Yii::t("app",'Pages').' '.$v->name; ?> 
<?php 
if( $user_satoo_this_page )
{
?>
<span style="font-size: 14px">คุณสาธุเพจนี้</span>
<?php }else{ ?>
<span onclick="playSatoo(); doSatooPage();">
...
<p><?=Yii::t("app",'Page Name') ?> : <?php echo $v->name; ?> | <?php echo Yii::t("app",'Page Category') ?> : <?php echo Yii::t("app", $v->category ); ?></p>
<p><?=Yii::t("app",'Page Description') ?> : <?php echo $v->description; ?> </p>
...
<?php 
// check owner can post only
if (  $gmodel->owner_id == Yii::app()->user->id ) { 
?>	
...
<?php } // end of if (  $gmodel->owner_id == Yii::app()->user->id )   ?>
